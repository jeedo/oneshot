...
- [ ] 47. Dockerfile: multi-stage build onto `mcr.microsoft.com/dotnet/aspnet:10.0-noble-chiseled` (non-root, no shell), read-only root filesystem, no volumes, `HEALTHCHECK` on `/healthz`; Trivy image scan and a syft SBOM in CI. (T5, T14)
